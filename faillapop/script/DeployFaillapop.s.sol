...
import {FP_CoolNFT} from "../src/FP_CoolNFTVulnerable.sol";
import {FP_DAO_Vulnerable} from "../src/FP_DAOVulnerable.sol";
import {FP_Token} from "../src/FP_ERC20Vulnerable.sol";
import {FP_Proxy} from "../src/FP_ProxyVulnerable.sol";
...
contract DeployFaillapop is Script {
    ...
    function run() external returns(FP_Shop shop, FP_Token token, FP_CoolNFT coolNFT, FP_PowersellerNFT powersellerNFT, FP_DAO_Vulnerable dao, FP_Vault vault, FP_Proxy proxy) {
        vm.startBroadcast(PRIVATE_KEY);
        shop = new FP_Shop();
        token = new FP_Token();
        coolNFT = new FP_CoolNFT();
        powersellerNFT = new FP_PowersellerNFT();
        dao = new FP_DAO_Vulnerable(token);
