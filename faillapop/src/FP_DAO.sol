...
contract FP_DAO is IFP_DAO, AccessControl {
    ...
    /**
        @notice Enum representing the possible votes
    */
    enum Vote {
        DIDNT_VOTE,
        COMMITTED,
    ...
    /**
        @notice Enum representing the state of a dispute
    */
    enum DisputeState {
        COMMITTING_PHASE,
        REVEALING_PHASE,
    ...
    /**
        @notice Enum representing the state of an upgrade proposal
    */
    enum ProposalState {
        ACTIVE,
        PASSED,
    ...
    */
    struct Dispute {
        uint256 itemId;                     // ID of the item in dispute
        string buyerReasoning;              // Buyer's reasoning for the dispute
        string sellerReasoning;             // Seller's response to the dispute
        uint256 votesFor;                   // Number of votes in favor of buyer
        uint256 votesAgainst;               // Number of votes in favor of seller
        uint256 totalCommits;               // Total number of commits
        uint256 totalVoters;                // Total number of voters
        uint256 committingStartingTime;     // When the dispute started (committing phase)
        uint256 revealingStartingTime;      // When the reveal phase started
        DisputeState state;                 // Current state of the dispute
    }

    /**
        @notice Struct representing an upgrade proposal
    */
    struct UpgradeProposal {
        address proposer;               // Address of the proposer
        uint256 proposalId;             // ID of the proposal
        uint256 creationTimestamp;      // When the proposal was created
        uint256 totalVoters;            // Total number of voters
        ...
        uint256 approvalTimestamp;      // When the proposal was approved
    ...
    /**
        ...
        @param disputeId The ID of the new dispute
        @param itemId The ID of the item in dispute
    */
    event NewDispute(uint256 indexed disputeId, uint256 indexed itemId);

    /**
        @notice Event emitted when a user commits a vote on a dispute
        @param disputeId The ID of the dispute
        @param voter The address of the voter
    */
    event DisputeVoteCommitted(uint256 indexed disputeId, address indexed voter);

    /**
        @notice Event emitted when a user reveals their vote on a dispute
        @param disputeId The ID of the dispute
        @param voter The address of the voter
    */
    event DisputeVoteCasted(uint256 indexed disputeId, address indexed voter);

    /**
        @notice Event emitted when a dispute is ended
        @param disputeId The ID of the dispute
        @param itemId The ID of the item that was in dispute
    */
    event EndDispute(uint256 indexed disputeId, uint256 indexed itemId);

    /**
        @notice Event emitted when a new upgrade proposal is created
        @param proposalId The ID of the new proposal
        @param timestamp When the proposal was created
        @param newImplementation The address of the new implementation
    */
    event NewUpgradeProposal(uint256 indexed proposalId, uint256 timestamp, address indexed newImplementation);

    /**
        @notice Event emitted when a user votes on an upgrade proposal
        @param proposalId The ID of the proposal
        @param voter The address of the voter
    */
    event ProposalVoteCasted(uint256 indexed proposalId, address indexed voter);

    /**
        @notice Event emitted when a proposal passes
        @param proposalId The ID of the proposal
        @param newImplementation The address of the new implementation
        @param timestamp When the proposal passed
    */
    event ProposalPassed(uint256 indexed proposalId, address indexed newImplementation, uint256 timestamp);

    /**
        @notice Event emitted when a proposal does not pass
        @param proposalId The ID of the proposal
        @param newImplementation The address of the new implementation
    */
    event ProposalNotPassed(uint256 indexed proposalId, address indexed newImplementation);
    ...
    /**
        ...
        @param newImplementation The address of the new implementation
    */
    event ProposalExecuted(uint256 indexed proposalId, address indexed newImplementation);
    ...
    */
    ...
    /**
        @notice Event emitted when a user is awarded an NFT
        @param user The address of the user
    */
    event AwardNFT(address indexed user);
    ...
    */
    ...
    function revealDisputeVote(uint disputeId, bool vote, string calldata secret) external {
        if(disputes[disputeId].state != DisputeState.REVEALING_PHASE) {
            if(disputes[disputeId].committingStartingTime + COMMITTING_TIME <= block.timestamp) {
    ...
    function cancelProposalByCreator(uint256 proposalId) external {  
        require(upgradeProposals[proposalId].state == ProposalState.ACTIVE, "Proposal is not active");
        require(upgradeProposals[proposalId].proposer == msg.sender, "You are not the creator of the proposal");
    ...
    function resolveUpgradeProposal(uint256 proposalId) external { 
        ...
        address newShop = upgradeProposals[proposalId].newImplementation;
